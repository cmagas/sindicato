...
abstract class  PdfObjectBase		// extends  Object
   {
	...
	protected function  IsFontMap ( $object_data )
	   {
		if  ( preg_match ( '#<< \s* ( (/F \d+) | (/f-\d+-\d+) | (/C\d+_\d+) ) \s+ .* >>#msx', $object_data ) )
			return ( true ) ;
		else
			return ( false ) ;
	    }
		...
		for ( $i = 0 ; $i < count ( $object_matches [ 'object' ] ) ; $i ++ )
		   {
			...
			if  ( preg_match ( '#/Length 0#i', $object_data ) )
				continue ;
		...
		switch  ( $type )
		   {
		   	case 	self::PDF_FLATE_ENCODING :
				...
				if  ( $decoded_stream_data  ===  false )
				   {
					if  ( self::$DEBUG  >  1 )
						throw ( new PdfToTextException ( "Invalid gzip data.", $object_id ) ) ;
				    }
							...
							// Characters are plain text
			   			   	else
							   {
								...
								for  ( $i = 0, $length = strlen ( $text ) ; $i  <  $length ; $i ++ )
									$result		.=  $this -> FontTable -> MapCharacter ( $current_font, $text [$i] ) ;
							    }
		...
		while  ( ( $part = $this -> __next_token ( $data, $data_length, $index ) )  !==  false )
		   {
			...
			if  ( ctype_digit ( $token [0] )  ||  $token [0]  ==  '-'  ||  $token [0]  ==  '+'  ||  $token [0]  ==  '.' )
				$number_stack []	=  $token ;
			...
			// "/R" : Specifies the font to be used for the next text output(s)
			else if  ( ! strncasecmp ( $token, '/R', 2 ) )
			   {
				...
				return ( [ 'instruction' => 'resource', 'next' => $next_index, 'resource' => $resource, 'token' => $token ] ) ;
			    }
		   	// "/F" : Specifies the font to be used for the next text output(s), using an indirect id
			else if  ( ! strncasecmp ( $token, '/F', 2 )  ||  ! strncasecmp ( $token, '/C', 2 ) )
			   {
			...
			// Other cases : this may be either a floating-point number or a keyword
		   	default :
		   		...
		   		if ( ctype_digit ( $ch )  ||  $ch  ==  '-'  ||  $ch  ==  '+' )
		   		   ...
		   		   {
					while  ( $index  <  $data_length  &&  
						( ctype_alnum ( $data [ $index ] )  ||  
							$data [ $index ]  ==  '*'  ||  $data [ $index ]  ==  '-'  ||  $data [ $index ]  ==  '_' ) )
						$value 	.=  $data [ $index ++ ] ;
		   		    }
	...
	protected function  PeekAuthorInformation ( $object_id, $object_data )
	   {
		...
		if  ( strpos  ( $object_data, '/Author' )  !==  false  &&  preg_match_all ( $re, $object_data, $matches ) )
		   {
	...
	public function  MapCharacter ( $ch )
	   {
		if  ( $this -> CharacterMap )
		...
		else if  ( $this -> FontType  ==  self::FONT_ENCODING_WINANSI )
		   {
			...
			if  ( isset ( self::$WinAnsiCharacterMap [ $ord ] ) )
				return ( $this -> CodePointToUtf8 ( self::$WinAnsiCharacterMap [ $ord ] ) ) ;
		    }  
		else if  ( $this -> FontType  ==  self::FONT_ENCODING_MAC_ROMAN )
		   {
			$ord	=  ord ( $ch ) ;

			if  ( isset ( self::$MacRomanCharacterMap [ $ord ] ) )
				return ( $this -> CodePointToUtf8 ( self::$MacRomanCharacterMap [ $ord ] ) ) ;
		    }  

		return ( $this -> CodePointToUtf8 ( ord ( $ch ) ) ) ;
	    }
    }
	...
	public function  offsetGet ( $offset )
	   {
		if  ( isset ( $this -> Map [ $offset ] ) )
			return ( $this -> CodePointToUtf8 ( $this -> Map [ $offset ] ) ) ;
		else
			return ( $this -> CodePointToUtf8 ( $offset ) ) ;
	    }
    }
			...
			if  ( $this -> GetObjectReferences ( $object_id, $object_data, '/Kids', $references ) )
			   {
				...
				if  ( $page_count  !==  false  &&  $reference_count  !=  $page_count  &&  PdfToText::$DEBUG )
				   {
